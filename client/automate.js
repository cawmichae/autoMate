(function () {
  ...
  const L = {
    ...
    PROCEED_TO_CHECKOUT_BUTTON_CART_PAGE: `cx-progress-button button`,
    ...
    CART: `cx-mini-cart a cx-icon`,
    FINAL_T_AND_C: `[formcontrolname="termsAndConditions"]`,
    PLACE_ORDER: `cx-place-order button.btn-primary`,
    SET_PREFERRED_STORE: `[data-preferred-store]`,
    CLOSE_DIALOG: `button.cx-dialog-close`,
  };
...

  {
    async function runJourney() {
      ...
      //   elements = await waitForElements(L.FIRST_PRODUCT);
      //   ELEMENTS = Array.from(elements);
      //   element = ELEMENTS[ELEMENTS.length - 1];
      //   console.log('Click on Last Product', element);
      //   element.click();
      ...
      //   elements = await waitForElements(L.ACTIVE_PICK_UP_IN_STORE_BUTTON)
      ...
      element = await waitForElement(L.SIGN_IN_LINK);
      console.log('Click on Sign In / Register Link', element);
      element.click();
      ...
      element = await waitForElement(`${L.FIRST_PRODUCT}[href*="45572"]`);
      ...
      element = await waitForElement(L.SET_PREFERRED_STORE);
      console.log('Click on Set Preferred Store');
      element.click();

      element = await waitForElement(L.CLOSE_DIALOG);
      console.log('Click on Close Dialog');
      element.click();

      //   element = await waitForElement(L.CART);
      //   await new Promise((r) => setTimeout(r, 1000));
      //   console.log('Click Cart');
      //   element.click();

      //   element = await waitForElement(L.PROCEED_TO_CHECKOUT_BUTTON_CART_PAGE);
      ...
      //   element = await waitForElement(L.FINAL_T_AND_C);
      //   await new Promise((r) => setTimeout(r, 1000));
      //   console.log('Click Final T and C', element);
      //   element.click();
      //   await new Promise((r) => setTimeout(r, 1000));

      //   element = await waitForElement(L.PLACE_ORDER)
      //   await new Promise((r) => setTimeout(r, 1000))
      //   console.log('Click an Place Order', element)
      //   element.click()
